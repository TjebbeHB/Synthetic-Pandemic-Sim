...
const NetherlandsMacroView: ComponentType | null = TABLET_BUILD
  ? null
  : lazy(() => import("./components/NetherlandsMacroView"));
...
type ViewMode = "agent" | "cellular" | "surveillance" | "rotterdam" | "netherlands";
...
export default function App({ initialView = "agent" }: { initialView?: ViewMode }) {
  ...
  useEffect(() => {
    if (TABLET_BUILD && (viewMode === "cellular" || viewMode === "rotterdam" || viewMode === "netherlands")) {
      setViewMode("agent");
    }
  }, [viewMode]);
    ...
    <main className="app">
      <header className="appHeader">
        ...
        <nav className="viewTabs" aria-label="Simulation view">
          ...
          {NetherlandsMacroView && (
            <button className={`heavyTab ${viewMode === "netherlands" ? "active" : ""}`} onClick={() => setViewMode("netherlands")}>
              Netherlands macro
            </button>
          )}
        ...
        <div className="headerStats">
          ...
          ) : viewMode === "netherlands" ? (
            <>
              <span>Netherlands macro model</span>
              <span>509 density clusters · rail-coupled</span>
              <span>metapopulation SEIRD</span>
            </>
          ) : viewMode !== "cellular" ? (
            <>
      ...
      ) : viewMode === "netherlands" && NetherlandsMacroView ? (
        <Suspense fallback={<div className="lazyFallback">Loading…</div>}>
          <NetherlandsMacroView />
        </Suspense>
      ) : (
      <section className="dashboard">
        <aside className="controlPanel panel">
          ...
          <Accordion id="policy" title="Measures & interventions" open={openGroups} onToggle={toggleGroup}>
            <SliderControl
              label="Measures start"
            ...
            <SliderControl
              label="Mask mandate"
              value={draftConfig.maskMandate ?? 0}
              min={0}
              max={0.7}
              step={0.01}
              formatValue={(value) => formatPercent(value)}
              onChange={(value) => updateConfigValue(setDraftConfig, "maskMandate", value)}
            />
            <SliderControl
              label="School closure"
              value={draftConfig.schoolClosure ?? 0}
              min={0}
              max={1}
              step={0.01}
              formatValue={(value) => formatPercent(value)}
              onChange={(value) => updateConfigValue(setDraftConfig, "schoolClosure", value)}
            />
            <SliderControl
              label="Case isolation / test-trace"
              value={draftConfig.caseIsolation ?? 0}
              min={0}
              max={0.95}
              step={0.01}
              formatValue={(value) => formatPercent(value)}
              onChange={(value) => updateConfigValue(setDraftConfig, "caseIsolation", value)}
            />
            <SliderControl
              label="Travel / border restriction"
              value={draftConfig.travelReduction ?? 0}
              min={0}
              max={0.95}
              step={0.01}
              formatValue={(value) => formatPercent(value)}
              onChange={(value) => updateConfigValue(setDraftConfig, "travelReduction", value)}
            />
          </Accordion>
