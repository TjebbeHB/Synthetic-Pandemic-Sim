...
export interface ScenarioConfig {
  ...
  /** Non-pharmaceutical interventions, all activated at policyStartDay and scaled
   *  by each agent's compliance. Optional so older saved scenarios still load. */
  /** Mask mandate — fractional transmission cut on all non-household layers (0–0.7). */
  maskMandate?: number;
  /** School closure — closes schools, cutting child/teen daytime + event contacts
   *  and education-sector workplace contacts (0–1). */
  schoolClosure?: number;
  /** Case isolation + test-trace-isolate — share of a case's onward COMMUNITY
   *  transmission removed by detecting and isolating them (0–0.95). Household
   *  spread is unaffected (you still infect the people you live with). */
  caseIsolation?: number;
  /** Travel / border restriction — fractional cut to inter-area commute and
   *  long-distance route transmission (0–0.95). */
