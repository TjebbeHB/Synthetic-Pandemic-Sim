...
export function normaliseConfig(config: ScenarioConfig): ScenarioConfig {
  return {
    ...
    maskMandate: clamp(config.maskMandate ?? 0, 0, 0.7),
    schoolClosure: clamp(config.schoolClosure ?? 0, 0, 1),
    caseIsolation: clamp(config.caseIsolation ?? 0, 0, 0.95),
    travelReduction: clamp(config.travelReduction ?? 0, 0, 0.95),
  };
}
...
function countInfectiousPressure(
  world: World,
  states: Uint8Array,
  externalScale = 1,
): InfectiousPressure {
  ...
  // externalScale < 1 models case isolation / test-trace-isolate: a detected case
  // sheds far less to work, school, events and the wider community, but still
  // infects their own household (you cannot isolate from the people you live with).
  for (const agent of world.agents) {
    if (states[agent.id] !== STATE.infectious) continue;
    increment(byHousehold, agent.householdId);
    increment(byDaytime, agent.daytimeNodeId, externalScale);
    increment(byEvent, agent.eventNodeId, externalScale);
    if (agent.routeNodeId) increment(byRoute, agent.routeNodeId, externalScale);
    increment(byProfile, agent.homeProfileId, externalScale);
  }
...
export function susceptibleRisk(
  ...
  // Additional NPIs (all gated on policyStartDay, scaled by per-agent compliance).
  // Masks cut every non-household layer; travel limits cut the commute layer;
  // school closure cuts contacts for the school-attending population (children /
  // teens and education-sector workers).
  const maskScale = policyActive ? 1 - (config.maskMandate ?? 0) * agent.compliance : 1;
  const travelScale = policyActive ? 1 - (config.travelReduction ?? 0) * agent.compliance : 1;
  const isSchoolGoer = agent.ageBand === "0-14" || agent.ageGroup === "teen";
  const attendsSchool = isSchoolGoer || agent.workSector === "education";
  const schoolWorkScale = policyActive && attendsSchool ? 1 - (config.schoolClosure ?? 0) * agent.compliance : 1;
  const schoolEventScale = policyActive && isSchoolGoer ? 1 - (config.schoolClosure ?? 0) * agent.compliance : 1;
  ...
  const terms: Record<Layer, number> = {
    ...
    work: LAYER_COEFFICIENTS.work * config.mobilityIntensity * mobilityScale * maskScale * schoolWorkScale * daytimePressure,
    commute:
      LAYER_COEFFICIENTS.commute * config.mobilityIntensity * mobilityScale * maskScale * travelScale * routePressure,
    event: LAYER_COEFFICIENTS.event * config.eventIntensity * eventScale * maskScale * schoolEventScale * eventPressure,
    community:
      LAYER_COEFFICIENTS.community *
      (0.66 + config.mobilityIntensity * mobilityScale * 0.34) *
      maskScale *
      communityPressure,
  };
...
export function simulate(world: World, config: ScenarioConfig): SimulationResult {
  ...
  for (let day = 1; day <= normalisedConfig.maxDays; day += 1) {
    ...
    const externalInfectiousScale =
      day >= normalisedConfig.policyStartDay ? 1 - (normalisedConfig.caseIsolation ?? 0) : 1;
    const pressure = countInfectiousPressure(world, states, externalInfectiousScale);
