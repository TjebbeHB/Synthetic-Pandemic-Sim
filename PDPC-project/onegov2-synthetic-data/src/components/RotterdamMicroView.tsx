...
// ---- measures presets ------------------------------------------------------
// Each preset sets the open-society mixing levels AND the full non-pharmaceutical
// intervention package (masks, schools, case isolation / test-trace, travel).
const SCENARIOS = {
  none: { label: "No measures", policyStartDay: 1000, mobilityReduction: 0, eventReduction: 0, eventIntensity: 0.95, mobilityIntensity: 1, maskMandate: 0, schoolClosure: 0, caseIsolation: 0, travelReduction: 0 },
  mild: { label: "Mild advisories", policyStartDay: 14, mobilityReduction: 0.35, eventReduction: 0.7, eventIntensity: 0.5, mobilityIntensity: 0.8, maskMandate: 0.35, schoolClosure: 0, caseIsolation: 0.3, travelReduction: 0.15 },
  schools: { label: "Schools + masks", policyStartDay: 10, mobilityReduction: 0.25, eventReduction: 0.6, eventIntensity: 0.55, mobilityIntensity: 0.85, maskMandate: 0.45, schoolClosure: 0.9, caseIsolation: 0.35, travelReduction: 0.1 },
  tti: { label: "Test, trace & isolate", policyStartDay: 8, mobilityReduction: 0.15, eventReduction: 0.45, eventIntensity: 0.7, mobilityIntensity: 0.9, maskMandate: 0.3, schoolClosure: 0, caseIsolation: 0.8, travelReduction: 0.1 },
  lockdown: { label: "Full lockdown", policyStartDay: 7, mobilityReduction: 0.8, eventReduction: 0.95, eventIntensity: 0.05, mobilityIntensity: 0.35, maskMandate: 0.55, schoolClosure: 0.95, caseIsolation: 0.5, travelReduction: 0.75 },
} as const;
...
export default function RotterdamMicroView() {
  ...
  const [maskMandate, setMaskMandate] = useState(0);
  const [schoolClosure, setSchoolClosure] = useState(0);
  const [caseIsolation, setCaseIsolation] = useState(0);
  ...
  function applyScenario(key: ScenarioKey) {
    ...
    setMaskMandate(s.maskMandate);
    setSchoolClosure(s.schoolClosure);
    setCaseIsolation(s.caseIsolation);
    setTravelReduction(s.travelReduction);
  }
  ...
  const draft = { infectionRate, incubationDays, infectiousDays, scenario, initialCases, areaZero, targetIFR, policyStartDay, mobilityReduction, eventReduction, maskMandate, schoolClosure, caseIsolation, travelReduction };
  ...
  const config: ScenarioConfig = useMemo(() => ({
    ...
    maskMandate: applied.maskMandate, schoolClosure: applied.schoolClosure, caseIsolation: applied.caseIsolation, travelReduction: applied.travelReduction,
  }), [applied, safeArea]);
    ...
    <section className="rotterdamMicro">
      ...
      <aside className="microPanel">
        ...
        <Accordion id="policy" title="Policy, healthcare & timing" open={openSection} setOpen={setOpenSection}>
          ...
          <Slider label="Event / gathering ban" value={eventReduction} min={0} max={0.98} step={0.01} fmt={(v) => `${Math.round(v * 100)}%`} onChange={setEventReduction} />
          <Slider label="😷 Mask mandate" value={maskMandate} min={0} max={0.7} step={0.01} fmt={(v) => `${Math.round(v * 100)}%`} onChange={setMaskMandate} />
          <Slider label="🏫 School closure" value={schoolClosure} min={0} max={1} step={0.01} fmt={(v) => `${Math.round(v * 100)}%`} onChange={setSchoolClosure} />
          <Slider label="🔬 Case isolation / test-trace" value={caseIsolation} min={0} max={0.95} step={0.01} fmt={(v) => `${Math.round(v * 100)}%`} onChange={setCaseIsolation} />
          <Slider label="✈️ Travel / border restriction" value={travelReduction} min={0} max={0.95} step={0.01} fmt={(v) => `${Math.round(v * 100)}%`} onChange={setTravelReduction} />
          <small className="microNote">Each measure activates on the start day and is scaled by per-agent compliance. Case isolation cuts community/workplace spread but not within-household transmission.</small>
