...
On top of the mobility/event/household intensities, the engine exposes an explicit
public-health intervention toolkit. All NPIs activate on `policyStartDay` and are
scaled by each agent's individual `compliance`, so the population effect is always
less than the nominal dial. Each one acts on the specific contact layers it would
plausibly touch — never the whole force of infection at once.

| Lever (`ScenarioConfig`) | Range | Layers affected | Mechanism |
|---|---|---|---|
| `maskMandate` | 0–0.7 | work, commute, event, community | Multiplies each non-household term by `1 − mask·compliance`. Households are unaffected (you don't mask at home). |
| `schoolClosure` | 0–1 | daytime + events of school-goers | Cuts the workplace/daytime term for children, teens and education-sector workers, and the event term for children/teens. |
| `caseIsolation` | 0–0.95 | community, work, event, commute | Symptomatic isolation + test-trace-isolate: an infectious case's *external* shedding is scaled by `1 − caseIsolation`, but its **household** transmission is untouched (you can't isolate from the people you live with). |
| `travelReduction` | 0–0.95 | commute / long-distance routes | Border and inter-city travel limits scale the commute term by `1 − travel·compliance`, the dominant inter-area transfer path in the national world. |

Because each lever hits a different layer, they compose realistically: masks +
school closure + test-trace each shave a slice, while a combined package (the
**Full lockdown** preset) stacks them into a strong suppression. The household
layer remains the hardest to suppress, which is why no single NPI drives the
epidemic to zero — matching the empirical experience of 2020–21.
